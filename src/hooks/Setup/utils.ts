import { empty_differences, empty_setup } from './assets'
...
const cleanLine = (FRFLRRRL: string, line: string): SetupProps => {
  ...
  const empty_line: SetupProps = { tab: '', key: '', name: '', value: '' }

  return empty_line
}

export const processSetupsContent = (setups: SetupCompleteSplitProps[]): SetupCompleteProps[] => {
...
export const readFiles = async (files_list: FileList): Promise<SetupCompleteRawProps[]> => {
...
export const splitSetupsContent = (files: SetupCompleteRawProps[]): SetupCompleteSplitProps[] =>
...
export const updateNumberOfDifferences = (
  setup_keys_to_show: SetupKeysToShowProps[],
  empty_setup: SetupProps[],
): DifferencesProps => {
  let new_differences: DifferencesProps = { ...empty_differences }
